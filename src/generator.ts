import { DMMF, generatorHandler, GeneratorManifest, GeneratorOptions } from '@prisma/generator-helper';
...
function createPaginateArgsDeclaration(modelName: string, cursorFields: string[], hasRelations: boolean) {
...
${indentString(
  models
    .map(
      (model) =>
        `this[${quote(lowerCase(model.name))}].paginate = paginate(this, ${quote(lowerCase(model.name))}, ${quote(defaults[model.name])});`
    )
    .join('\n'),
  8
)}
...
async function createTypescriptDefinitions(models: DMMF.Model[], outputDir: string) {
  const paginateArgsDeclarations = models.map((model) => {
    const cursorFields = model.fields.filter((field) => field.isUnique || field.isId).map((field) => field.name);
  ...
  const delegateExtensions = models.map((model) => createDelegateExtension(model.name));

  const imports = createTypeImports(models.map((model) => model.name));
  const moduleDefinition = createPrismaModuleDefinition([...paginateArgsDeclarations, ...delegateExtensions]);
